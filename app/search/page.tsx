...
export default async function SearchPage({
  ...
  return (
    <main className="min-h-screen bg-ivory text-deep-green flex flex-col font-sans" dir="rtl">
      <Navbar />

      <div className="flex-grow pt-32 pb-24 px-6 max-w-7xl mx-auto w-full">
        <div className="mb-10 text-center">
          <h1 className="text-3xl md:text-4xl font-black text-deep-green mb-4">
        ...
        {!query ? (
          <div className="flex flex-col items-center justify-center py-20 opacity-50">
            <Search className="w-16 h-16 text-deep-green mb-6" />
                ...
                <div className="relative aspect-[4/5] bg-white border border-black/5 rounded-2xl overflow-hidden mb-4 p-6">
                  {product.imageUrl ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <span className="text-gold text-2xl">طيف</span>
                    </div>
                  )}
                </div>
                
                <div className="flex flex-col flex-grow">
                  <div className="text-xs text-gold font-bold tracking-widest mb-2 uppercase">
                    {product.category || 'عطر'}
                  </div>
                  <h3 className="text-lg font-bold text-deep-green mb-2 group-hover:text-emerald transition-colors line-clamp-1">
                    {product.name}
                  </h3>
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white border border-black/5 rounded-3xl">
            <Filter className="w-16 h-16 text-deep-green/20 mb-6" />
            <h2 className="text-2xl font-bold text-deep-green mb-3">لم نجد أي نتائج!</h2>
            <p className="text-deep-green/60 text-center max-w-md mb-8">
              لم نتمكن من العثور على أي منتج يطابق "{query}". يرجى التأكد من الكلمات المستخدمة أو تجربة كلمات أخرى.
            </p>
            <Link 
              href="/products"
              className="bg-deep-green text-ivory px-8 py-3 rounded-none font-bold hover:bg-emerald transition-colors inline-flex items-center gap-2"
            >
              تصفح جميع المنتجات
