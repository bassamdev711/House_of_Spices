      ...
      <div className="flex-grow pt-32 pb-24 px-6 max-w-7xl mx-auto w-full flex justify-center items-start">
        <div className="w-full max-w-[640px] bg-white shadow-sm p-8 md:p-12 border border-black/5">
          ...
          <header className="mb-12 text-center">
            <h1 className="text-3xl font-black text-foreground mb-4">إثبات الدفع</h1>
            <p className="text-foreground/70">الرجاء تحويل المبلغ إلى الحساب التالي وإرفاق صورة إشعار التحويل.</p>
          </header>

          {/* Bank/Wallet Details */}
          <div className="bg-surface-alt border border-black/5 p-6 mb-10 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-1 h-full bg-brand"></div>
            <h3 className="text-sm font-bold text-brand mb-6 uppercase tracking-wider">الحسابات المتوفرة للتحويل</h3>
            ...
            <div className="space-y-6">
              {paymentData.bankAccounts.map((bank: any) => (
                <div key={bank.id} className="border-b border-black/5 pb-4 last:border-0 last:pb-0">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-foreground/70">اسم الحساب</span>
                    <span className="font-bold">{bank.accountName}</span>
                  </div>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-foreground/70">البنك</span>
                    <span className="font-bold">{bank.bankName}</span>
                  </div>
                  <div className="flex justify-between items-center flex-wrap gap-4">
                    <span className="text-foreground/70">رقم الحساب / IBAN</span>
                    <div className="flex items-center gap-2">
                      <span className="font-bold tracking-widest text-sm" dir="ltr">{bank.accountNumber}</span>
                      <button onClick={() => handleCopy(bank.accountNumber)} className="text-foreground/40 hover:text-brand transition-colors" title="نسخ">
                        <Copy size={16} />
                      </button>
                    </div>
                  </div>
              ...
              {paymentData.digitalWallets.map((wallet: any) => (
                <div key={wallet.id} className="border-b border-black/5 pb-4 last:border-0 last:pb-0">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-foreground/70">المحفظة</span>
                    <span className="font-bold">{wallet.walletName}</span>
                  </div>
                  <div className="flex justify-between items-center flex-wrap gap-4">
                    <span className="text-foreground/70">رقم الجوال / الحساب</span>
                    <div className="flex items-center gap-2">
                      <span className="font-bold tracking-widest text-sm" dir="ltr">{wallet.accountNumber}</span>
                      <button onClick={() => handleCopy(wallet.accountNumber)} className="text-foreground/40 hover:text-brand transition-colors" title="نسخ">
                        <Copy size={16} />
                      </button>
                    </div>
                  </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            {/* Upload Area */}
            <div>
              <label className="block text-sm font-bold text-foreground mb-4">صورة إشعار التحويل</label>
              <div 
                onClick={() => fileInputRef.current?.click()}
                className={`border-2 border-dashed ${file ? 'border-brand bg-brand/5' : 'border-black/20 hover:border-brand bg-surface-alt'} p-10 flex flex-col items-center justify-center cursor-pointer transition-all duration-300 group`}
              >
                <UploadCloud className={`w-12 h-12 mb-4 transition-colors ${file ? 'text-brand' : 'text-foreground/40 group-hover:text-brand'}`} />
                <p className="text-foreground text-center mb-2">
                  {file ? <span className="font-bold text-brand">{file.name}</span> : <span>اسحب وأفلت الصورة هنا، أو <span className="text-brand font-bold">استعرض</span></span>}
                </p>
            ...
            <div className="flex flex-col">
              <label className="text-sm font-bold text-foreground mb-2">رقم العملية (اختياري)</label>
              <input 
                type="text" 
                value={transactionId}
            ...
            <button 
              ...
              className="w-full bg-accent text-foreground border border-black font-bold py-5 flex justify-center items-center gap-3 hover:bg-accent transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed rounded-none"
            >
